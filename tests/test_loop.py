...
class TestLoop(unittest.TestCase):
    def test_switch_task_end_to_end(self):
        ...
        planner = Planner(Script([
            {"thought": "关掉深色主题", "action": "click", "target": sid},
            {"thought": "已经是 Off 了", "action": "finish", "done": True},
        ]))
        with tempfile.TemporaryDirectory() as d:
            ...
            # 开关真的翻转了，且判据是 PASS
            self.assertFalse(tp.nodes[10]["checked"])
    ...
    def test_restore_failure_is_surfaced_not_swallowed(self):
        ...
        obs = build_observation("关闭深色主题", env, [], [step])
